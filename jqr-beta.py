...
addmsg('欢迎使用机器人快捷操作系统，你可以使用此系统完成对机器人的常用操作，每次更新都会有新功能！ 由于时间仓促，代码难免会出现问题，如遇到问题，请给王大哥私信，请保持你的软件版本为最新版本 最新下载：http://fanbook_wdgsys.bailituya.com/ [王大哥 V3.6 让机器人控制变得更简单！]',color='aqua')
url = 'http://fanbook_wdgsys.bailituya.com/data.txt'#获取版本数据
response = urllib.request.urlopen(url)
data = response.read()
datatext = data.decode('utf-8') 
print("最新版本：",datatext)
if float(datatext) > 3.6:
...
while True:
    try:#检测代码，防止错误闪退
        colorprint(smg2='请选择模式，1为发送消息(可发送私信消息)，2为发送图片，3为创建私聊频道，4为禁言用户，5为创建频道，6为通过消息链接获取消息详细信息，7为反馈模式，8为富文本模式，9为为成员添加自定义勋章，10为获取成员勋章的详细信息，命令行启动命令行模式，help获取错误码帮助',pcolor='bandg')
        a=input()
        if a=='1':
            ...
            c=input()
            d='{\"type\":\"richText\",\"title\":\"反馈 V3.6'+'\",\"document\":\"[{\\\"insert\\\":\\\"'+c+'\\\"}]\"}'
            url='https://a1.fanbook.mobi/api/bot/0f2de7ac66727cd9fcec1eecf94ab795fbfbe39ad311a18ad1ff314388d1c/sendMessage'
            headers = {'content-type':"application/json;charset=utf-8"}
            ...
            while True:
                for event in pygame.event.get():
                    if event.type == pygame.KEYDOWN and event.key == pygame.K_RETURN:
                        ...
                        addmsg('—————————————————————————————————————————————',color='aqua')
                        addmsg('关于：王大哥（#4562997）V3.6版',color='aqua')
                        addmsg('更新日期：2023/4/6  15：30 ',color='aqua')
        elif a== '8':
            colorprint(smg2='请输入频道id（如私信需要私聊id,可通过获取私聊id获取），获取方法：聊天框输入#，然后选择频道，发送后复制刚刚发送的蓝色频道名，复制后例如${#395848618357086556}，填写里面的数字395848618357086556即可',pcolor='bandg')
            pdid=input()
            ...
            wbnr=input()
            colorprint(smg2='勋章图标个数（1~2）',pcolor='bandg')
            tbgs=input()
            if tbgs=="2":
        ...
        if 'pygame.error: video system not initialized' in error:#忽略错误
            ...
            cw=input()
            if cw=='Y':
                    cwbg='错误模块：'+a+' 版本号：3.6'+'，错误代码：'+error
                    xwxx='{\"type\":\"richText\",\"title\":\"错误报告'+'3.1'+'\",\"document\":\"[{\\\"insert\\\":\\\"'+' '+'\\\"}]\"}'#此段富文本不支持
                    url='https://a1.fanbook.mobi/api/bot/0f2de7ac66727cd9fcec1ee43559c561fe39ad311a18ad1ff314388d1c/sendMessage'#错误发送到私密频道
                    headers = {'content-type':"application/json;charset=utf-8"}
