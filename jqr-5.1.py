...
try:
    ...
    def main():
        ...
        url = "http://fanbookwdg.qhla.eu.org/Update.exe"  # 替换成你的文件下载链接
...
try:
    url = 'http://fanbookwdg.qhla.eu.org/data.txt'#获取版本数据
    response = urllib.request.urlopen(url)
    data = response.read()
    datatext = data.decode('utf-8') 
    url = 'http://fanbookwdg.qhla.eu.org/datab.txt'#获取公告数据
    response = urllib.request.urlopen(url)
    data = response.read()
    ...
    if float(datatext) > v:
        print("有最新版本,即将更新，或者请去 http://fanbookwdg.qhla.eu.org/ 下载最新版本")
    ...
    jsonfile=json.dumps({"chat_id":448843628261933056,"text":'版本:'+str(v)+'操作系统: {} {}'.format(os_name, os_version)+' 架构: {}'.format(os_arch)+' 计算机名: {}'.format(computer_name)+' CPU负载: {}'.format(cpu_usage)})
    postreturn=requests.post(url,data=jsonfile,headers=headers)
